...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar"><button className="brand" onClick={()=>setView("live")} aria-label="FactoryTrace home"><span className="brand-mark">FT</span><span><strong>FACTORYTRACE</strong><small>Traceability digital twin</small></span></button><div className="line-identity"><span>PT NUSA VITA NUTRINDO</span><b>PKG—02</b></div><div className="status-cluster"><span className={`status-pill ${sim.lineState === "RUNNING"?"running":sim.lineState === "EMERGENCY_STOP"?"emergency":"stopped"}`}><i/>{sim.lineState === "EMERGENCY_STOP" ? "SAFETY · E-STOP" : `AUTO · ${sim.lineState}`}</span><span className={`status-pill ${sim.opcConnected ? "running" : "stopped"}`}><i/>{sim.opcConnected ? "OPC UA · CONNECTED" : "BROWSER SIM · FALLBACK"}</span><time>{time} WIB</time></div></header>
    ...
    <section className={`workspace view-${view}`} id="top">{view==="live"&&<LiveLine sim={sim}/>} {view==="trace"&&<TraceExplorer products={[...sim.activeProducts,...sim.recentProducts]}/>} {view==="quality"&&<QualityView products={sim.recentProducts} alarms={sim.alarms} acknowledge={sim.acknowledge}/>} {view==="system"&&<SystemMap opcConnected={sim.opcConnected} integrationMode={sim.integrationMode}/>}</section>
