...
import { InterfaceLines } from "@designcodeio/threeui/components/InterfaceLines";
...
export function LiveLine({ sim }:{sim:Simulator}) {
  ...
  const rejectingProduct = sim.activeProducts.find((product) => product.disposition === "REJECT" && product.position >= 82);
  const queuedProduct = sim.activeProducts.find((product) => product.disposition === "REJECT" && product.position < 82);
  const effectiveCamera = rejectingProduct ? "reject" : camera;
  return <>
    ...
    <section className="hero-grid">
      <article className="digital-twin-card">
        <div className="panel-label"><span>DIGITAL TWIN / {effectiveCamera.toUpperCase()}</span><div className="camera-tabs"><button onClick={() => setCamera("overview")} className={effectiveCamera === "overview" ? "selected":""}>OVERVIEW</button><button onClick={() => setCamera("inspection")} className={effectiveCamera === "inspection" ? "selected":""}>INSPECTION</button><button onClick={() => setCamera("reject")} className={effectiveCamera === "reject" ? "selected":""}>REJECT</button></div></div>
        <div className="factory-canvas"><FactoryScene products={sim.activeProducts} running={sim.lineState === "RUNNING"} cameraPreset={effectiveCamera}/><InterfaceLines mode="dark" speed={.28} density={.38} strokeWidth={.65} opacity={.28} hue={68} saturation={1.2} brightness={1.08} className="threeui-signal-layer"/><div className="station-overlay"><span><i/>ENTRY / S01</span><span><i/>VISION / S02</span><span><i/>BARCODE / S03</span><span><i/>REJECT / S04</span></div><div className="viewport-title"><span>PKG—02 / SPATIAL PROCESS TWIN</span><b>{sim.opcConnected?"LIVE OPC UA":"LOCAL FALLBACK"}</b></div>{sim.lineState === "EMERGENCY_STOP" ? <div className="safety-trip-overlay"><i>!</i><span><b>EMERGENCY STOP LATCHED</b><small>All simulated motion and outputs inhibited</small></span></div> : null}<div className={`reject-process ${rejectingProduct ? "active" : queuedProduct ? "queued" : "armed"}`}><i/><span>{rejectingProduct ? "PUSHER ACTIVE" : queuedProduct ? "REJECT QUEUED" : "S04 ARMED"}</span><b>{rejectingProduct?.serial ?? queuedProduct?.serial ?? "Waiting for reject target"}</b></div><div className="canvas-hint">DRAG TO ORBIT · SCROLL TO ZOOM</div></div>
...
export function SystemMap({opcConnected,integrationMode}:{opcConnected:boolean;integrationMode:"BROWSER_SIM"|"OPC_UA"}) {
  const tags=["Line.State","Line.SpeedActual","Order.Number","Product.Sequence","Product.Serial","Product.Position","Product.VisionResult","Product.BarcodeResult","Product.Disposition","Safety.EStopLatched","Command.StartRequest","Command.StopRequest","Command.EmergencyStopRequest","Command.ResetSafetyRequest","Command.InjectScenario","Command.AcknowledgeAlarm"];
  const status=opcConnected?"LIVE LOCAL OPC UA SESSION":integrationMode==="OPC_UA"?"OPC UA RECONNECTING":"BROWSER FALLBACK · OPC UA OFFLINE";
  return <><div className="section-heading"><div><p className="eyebrow">OT / IT INTEGRATION CONTRACT</p><h1>System architecture</h1></div><span className={`simulation-disclaimer ${opcConnected?"connected":""}`}>{status}</span></div><section className="opc-proof"><div><span>RUNTIME MODE</span><b>{integrationMode}</b></div><div><span>OPC UA ENDPOINT</span><b>opc.tcp://127.0.0.1:4840/FactoryTrace</b></div><div><span>BACKEND CLIENT</span><b>{opcConnected?"SUBSCRIBED · 100 ms":"NOT CONNECTED"}</b></div><div><span>LOCAL SECURITY</span><b>SecurityPolicy None · demo only</b></div></section><section className="architecture-card"><div className="architecture-layer field"><label>LEVEL 0—1 / FIELD & CONTROL</label><div className="arch-nodes"><div className="arch-node"><span>SIMULATED SENSOR</span><b>Keyence IV3-G120</b><small>Vision · cap & label judgment</small><em>Modeled field input</em></div><div className="arch-node primary"><span>PLC SIMULATOR / OPC UA SERVER</span><b>KV-8000A behavior model</b><small>Sequence · interlock · product FIFO</small><em>opc.tcp://127.0.0.1:4840/FactoryTrace</em></div><div className="arch-node"><span>SIMULATED CODE READER</span><b>Keyence SR-1000</b><small>Serialized 2D code verification</small><em>Modeled field input</em></div></div></div><div className="protocol-bridge"><i/><span>OPC UA subscription · 100 ms · local anonymous demo</span><i/></div><div className="architecture-layer it"><label>LEVEL 3 / TRACEABILITY</label><div className="arch-nodes two"><div className="arch-node backend"><span>REAL OPC UA CLIENT / BRIDGE</span><b>FactoryTrace backend</b><small>Subscribes to PLC nodes and writes command nodes</small><em>HTTP + Server-Sent Events to browser</em></div><div className="arch-node database"><span>PERSISTENCE</span><b>Product history database</b><small>Units · inspections · events · alarms</small><em>Append-only product events</em></div></div></div></section><section className="tag-contract"><div><p className="eyebrow">OPC UA ADDRESS SPACE</p><h2>PKG-02 tag contract</h2><p>These are live OPC UA Variable nodes when the local simulator is running. The web app consumes them through the backend bridge; it does not open raw OPC TCP from the browser.</p></div><div className="tag-list">{tags.map((tag,i)=><code key={tag}><span>{String(i+1).padStart(2,"0")}</span>FactoryTrace/PKG02/{tag.replace(".","/")}</code>)}</div></section></>;
}

export function ControlDock({ sim }:{sim:Simulator}) {
  const emergency = sim.lineState === "EMERGENCY_STOP";
  const controlsAvailable = sim.integrationMode !== "OPC_UA" || sim.opcConnected;
  const modeLabel = sim.opcConnected ? "OPC UA MODE" : sim.integrationMode === "OPC_UA" ? "OPC UA RECONNECTING" : "BROWSER FALLBACK";
  return <footer className={`control-dock ${emergency ? "safety-latched" : ""}`}><button className="emergency-stop" onClick={sim.emergencyStop} disabled={emergency||!controlsAvailable} aria-label="Activate simulated emergency stop"><i>STOP</i><span>E—STOP</span></button><div className="control-buttons">{emergency?<button className="safety-reset" onClick={sim.resetSafety} disabled={!controlsAvailable}>↻ RESET SAFETY</button>:<><button className={sim.lineState === "RUNNING" ? "stop" : "start"} onClick={()=>sim.setLineState(sim.lineState === "RUNNING" ? "STOPPED" : "RUNNING")} disabled={!controlsAvailable}>{sim.lineState === "RUNNING" ? "■ CONTROLLED STOP" : "▶ START LINE"}</button><button onClick={sim.reset} disabled={!controlsAvailable}>↻ RESET SIM</button></>}</div><label className="scenario-picker" htmlFor="scenario"><span>NEXT PRODUCT SCENARIO</span><select id="scenario" value={sim.scenario} onChange={(e)=>sim.setScenario(e.target.value as Scenario)} disabled={!controlsAvailable}>{(Object.keys(scenarioLabels) as Scenario[]).map((key)=><option value={key} key={key}>{scenarioLabels[key]}</option>)}</select></label><button className="inject" onClick={sim.inject} disabled={sim.lineState!=="RUNNING"||!controlsAvailable}>＋ INJECT PRODUCT</button><p>{modeLabel}<span>E-stop is not a safety device</span></p></footer>;
}
